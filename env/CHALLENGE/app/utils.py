...
def ingest_data_from_csv(file_path):
    ...
    for _, row in data.iterrows():
        trips_data = Trip(region=row['region'], origin_coord=row['origin_coord'], destination_coord=row['destination_coord'], datetime=row['datetime'], datasource=row['datasource'],group_id=row['group_id'])
        session.add(trips_data)
